...
export default function DashboardReservations() {
  ...
  const handleConfirmAndSendTickets = async (reservation: any) => {
        ...
        : [
            {
              ...
              ticket_type:
                reservation.single_count > 0
                  ? 'single'
                  : reservation.standing_count > 0
                  ? 'standing'
                  : reservation.backstage_count > 0
                  ? 'backstage'
                  : reservation.vip_count > 0
                  ? 'vip'
                  : 'single',
            },
          ]
    ...
    const ticketsToInsert = allPeople.map((person: any, index: number) => ({
      ...
      ticket_type: person.ticket_type || 'single',
  ...
  if (selected) {
    ...
    const summaryCards = [
      {
        label: 'TOTAL PEOPLE',
        value: `${selected.num_people || selected.quantity} person${
          (selected.num_people || selected.quantity) > 1 ? 's' : ''
        }`,
        color: '#fff',
      },
      ...(singleCount > 0
        ? [
            {
              label: 'SINGLE',
              value: `${singleCount}x @ ${selected.single_price_per_person ?? '-'} EGP`,
              color: '#60a5fa',
            },
          ]
        : []),
      ...(standingCount > 0
        ? [
            {
              label: 'STANDING',
              value: `${standingCount}x @ ${selected.standing_price_per_person ?? '-'} EGP`,
              color: '#27AE60',
            },
          ]
        : []),
      ...(backstageCount > 0
        ? [
            {
              label: 'BACKSTAGE',
              value: `${backstageCount}x @ ${selected.backstage_price_per_person ?? '-'} EGP`,
              color: '#8b5cf6',
            },
          ]
        : []),
      ...(vipCount > 0
        ? [
            {
              label: 'VIP',
              value: `${vipCount}x @ ${selected.vip_price_per_person ?? '-'} EGP`,
              color: '#F0A500',
            },
          ]
        : []),
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '96px 24px 80px' }}>
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(46,117,182,0.12)', borderRadius: 20, padding: 32 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
              <div>
                <h2 style={{ color: '#fff', fontSize: 24, fontWeight: 800, margin: '0 0 6px', fontFamily: 'Poppins, sans-serif' }}>
              ...
              <div
                style={{
                  background: `${statusColors[selected.status] || '#555'}18`,
                  ...
                  <div key={step.key} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div
                      style={{
                        ...
                        fontWeight: 700,
                        letterSpacing: '1px',
                        background: isCurrent ? `${statusColors[step.key]}20` : isDone ? 'rgba(39,174,96,0.1)' : 'rgba(255,255,255,0.03)',
                        border: `1px solid ${
                          isCurrent ? statusColors[step.key] : isDone ? 'rgba(39,174,96,0.4)' : 'rgba(255,255,255,0.06)'
                        }`,
                        color: isCurrent ? statusColors[step.key] : isDone ? '#27AE60' : 'rgba(255,255,255,0.2)',
                      }}
                    >
                      {isDone ? '✓ ' : ''}
                      {step.label}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${summaryCards.length}, 1fr)`, gap: 12, marginBottom: 24 }}>
              {summaryCards.map(card => (
                ...
                >
                  <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: 10, letterSpacing: '2px', fontWeight: 700, margin: '0 0 6px' }}>
                    {card.label}
                  </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1.4fr 1.2fr', gap: 12, marginBottom: 24 }}>
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '14px 16px' }}>
                <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: 10, letterSpacing: '2px', fontWeight: 700, margin: '0 0 8px' }}>
                  MAIN GUEST
                </p>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '14px 16px' }}>
                ...
                {(() => {
                  ...
                  const wave =
                    type === 'single'
                      ? selected.single_wave_label
                      : type === 'backstage'
                      ? selected.backstage_wave_label
                      : type === 'vip'
                      ? selected.vip_wave_label
                      : selected.standing_wave_label
            ...
            {guests.length > 0 && (
              ...
              >
                <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: 10, letterSpacing: '2px', fontWeight: 700, margin: '0 0 14px' }}>
                  ALL GUESTS ({guests.length})
                </p>
                <div
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '0.4fr 1.4fr 1fr 1fr 1fr',
                    gap: 10,
                    padding: '8px 10px',
            ...
            {selected.payment_sender_phone && (
              ...
              >
                ...
                <p style={{ color: '#2E75B6', fontSize: 22, fontWeight: 900, fontFamily: 'monospace', letterSpacing: '3px', margin: 0 }}>
                  {selected.payment_sender_phone}
                </p>
              </div>
            )}
              ...
              >
                ...
                <a href={selected.payment_screenshot_url} target="_blank" rel="noreferrer">
                  <img
                    ...
                    alt="Payment proof"
                    style={{ width: '100%', maxHeight: 300, objectFit: 'contain', borderRadius: 8, cursor: 'zoom-in' }}
                  />
                </a>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button onClick={() => updateStatus(selected.id, 'reviewing')} style={btnStyle('#fb923c', selected.status === 'reviewing')} disabled={loading}>
                  🔎 REVIEWING
                </button>
                <button
                  onClick={() => updateStatus(selected.id, 'awaiting_payment')}
                  style={btnStyle('#2E75B6', selected.status === 'awaiting_payment')}
                  disabled={loading}
                >
                  💳 REQUEST PAYMENT
                </button>
                <button
                  ...
                  style={{
                    ...
                    cursor: alreadySent || loading ? 'not-allowed' : 'pointer',
                  }}
                ...
                <button onClick={() => updateStatus(selected.id, 'rejected')} style={btnStyle('#E74C3C')} disabled={loading}>
                  ❌ REJECT
                </button>
              </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, overflow: 'hidden' }}>
          ...
          >
            {['GUEST', 'EVENT', 'TICKETS', 'TOTAL', 'STATUS'].map(h => (
              <span key={h} style={{ color: 'rgba(255,255,255,0.2)', fontSize: 10, letterSpacing: '2px', fontWeight: 700 }}>
                {h}
              </span>
            ))}
          </div>
          ...
          {filtered.map((r, i) => (
            ...
            >
              ...
              <div style={{ alignSelf: 'center' }}>
                <span
                  style={{
                    background: `${statusColors[r.status] || '#555'}15`,
