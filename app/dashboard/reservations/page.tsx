...
export default function DashboardReservations() {
  ...
  const handleConfirmAndSendTickets = async (reservation: any) => {
    ...
    const allPeople = people.length > 0 ? people : [{
      name: mainName, phone: mainPhone, instagram: mainInstagram,
      ticket_type: reservation.single_count > 0 ? 'single' : reservation.standing_count > 0 ? 'standing' : reservation.backstage_count > 0 ? 'backstage' : reservation.vip_count > 0 ? 'vip' : 'single',
    }]

    const ticketsToInsert = allPeople.map((person: any, index: number) => ({
      reservation_id: reservation.id, event_id: reservation.event_id, user_id: reservation.user_id,
      holder_name: person.name || mainName, holder_phone: person.phone || mainPhone || null,
      holder_instagram: person.instagram || mainInstagram || null,
      ticket_type: person.ticket_type || 'single', ticket_number: index + 1,
      qr_code: `TH-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}-${index}`,
    }))

    const { data: createdTickets, error: ticketsError } = await supabase.from('tickets').insert(ticketsToInsert).select()
    ...
    const eventDate = reservation.events?.date ? new Date(reservation.events.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 'TBA'
  ...
  const handleSendGmail = async (reservation: any) => {
    ...
    const eventDate = reservation.events?.date ? new Date(reservation.events.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 'TBA'
    const eventLocation = reservation.events?.location || 'TBA'
    const { data: tickets } = await supabase.from('tickets').select('*').eq('reservation_id', reservation.id).order('ticket_number', { ascending: true })
    const origin = window.location.origin
    const mainName = reservation.full_name || reservation.name || 'Guest'
    ...
    window.open(`https://mail.google.com/mail/?view=cm&to=${encodeURIComponent(reservation.email)}&su=${subject}&body=${body}`, '_blank')
  }
  ...
  if (selected) {
    ...
    const singleCount = selected.single_count ?? 0
    ...
    const summaryCards = [
      ...
      ...(singleCount > 0 ? [{ label: 'SINGLE', value: `${singleCount}x @ ${selected.single_price_per_person ?? '-'} EGP`, color: '#60a5fa' }] : []),
    ...
    const flowSteps = [
      { key: 'pending', label: '1. PENDING' }, { key: 'reviewing', label: '2. REVIEWING' },
      { key: 'awaiting_payment', label: '3. PAYMENT' }, { key: 'payment_review', label: '4. REVIEW' },
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(46,117,182,0.12)', borderRadius: 20, padding: 32 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                {isViewer && (
                  <span style={{ background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.2)', color: '#94a3b8', fontSize: 10, fontWeight: 700, letterSpacing: '1.5px', padding: '6px 14px', borderRadius: 8 }}>👁️ VIEW ONLY</span>
                )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1.4fr 1.2fr', gap: 12, marginBottom: 24 }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '14px 16px' }}>
                ...
                {(() => {
                  const type = mainGuest?.ticket_type || (singleCount > 0 ? 'single' : standingCount > 0 ? 'standing' : backstageCount > 0 ? 'backstage' : vipCount > 0 ? 'vip' : null)
                  const color = ticketTypeColor[type] || 'rgba(255,255,255,0.4)'
                  const wave = type === 'single' ? selected.single_wave_label : type === 'backstage' ? selected.backstage_wave_label : type === 'vip' ? selected.vip_wave_label : selected.standing_wave_label
                  return (
                    <>
                      <p style={{ color, fontSize: 16, fontWeight: 800, margin: '0 0 6px', fontFamily: 'Poppins, sans-serif' }}>{type === 'vip' ? '👑 ' : ''}{type ? type.toUpperCase() : 'N/A'}</p>
              ...
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 20, marginBottom: 24 }}>
                ...
                {guests.map((p: any, i: number) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '0.4fr 1.4fr 1fr 1fr 1fr', gap: 10, padding: '10px', borderBottom: i < guests.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', fontSize: 13, alignItems: 'center' }}>
                    ...
                    <span style={{ color: ticketTypeColor[p.ticket_type] || 'rgba(255,255,255,0.4)', fontWeight: 700 }}>{p.ticket_type === 'vip' ? '👑 ' : ''}{p.ticket_type ? p.ticket_type.toUpperCase() : 'UNKNOWN'}</span>
                    <span style={{ color: 'rgba(255,255,255,0.5)' }}>{p.phone}</span>
                    <span>{p.instagram ? (<a href={p.instagram} target="_blank" rel="noreferrer" style={{ color: '#e1306c', fontSize: 12, textDecoration: 'none', border: '1px solid rgba(225,48,108,0.3)', borderRadius: 6, padding: '3px 8px' }}>📸 Instagram</a>) : (<span style={{ color: 'rgba(255,255,255,0.15)' }}>—</span>)}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 24 }}>
              ...
              {isViewer ? (
                <div style={{ background: 'rgba(148,163,184,0.05)', border: '1px solid rgba(148,163,184,0.15)', borderRadius: 10, padding: '14px 20px' }}>
                  <p style={{ color: '#94a3b8', fontSize: 12, margin: 0 }}>👁️ You have view-only access. Actions are disabled.</p>
                </div>
              ) : (
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => handleConfirmAndSendTickets(selected)} style={{ ...btnStyle('#27AE60'), opacity: alreadySent ? 0.4 : loading ? 0.5 : 1, cursor: alreadySent || loading ? 'not-allowed' : 'pointer' }} disabled={loading || alreadySent}>🎫 CONFIRM + WHATSAPP</button>
                  {selected.email && (
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#0a0f1e', padding: 0, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '96px 24px 80px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {isViewer && (
              <span style={{ background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.2)', color: '#94a3b8', fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', padding: '8px 16px', borderRadius: 8 }}>👁️ VIEW ONLY</span>
            )}
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
          {['all', ...allStatuses].map(s => {
            ...
            return (
              <button key={s} onClick={() => setFilter(s)} style={{ background: isActive ? 'linear-gradient(135deg, #1A3C5E, #2E75B6)' : 'rgba(255,255,255,0.03)', color: isActive ? '#fff' : 'rgba(255,255,255,0.3)', border: `1px solid ${isActive ? 'transparent' : 'rgba(255,255,255,0.06)'}`, padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, letterSpacing: '1px', cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
            ...
            >
              ...
              <div style={{ alignSelf: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 3, flexWrap: 'wrap' }}>
                  {r.single_count > 0 && <span style={{ fontSize: 9, fontWeight: 700, color: '#60a5fa', background: 'rgba(96,165,250,0.1)', border: '1px solid rgba(96,165,250,0.2)', borderRadius: 4, padding: '1px 5px' }}>SGL×{r.single_count}</span>}
