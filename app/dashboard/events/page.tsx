...
export default function DashboardEvents() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: any = {
      ...
      single_wave_1_price: form.single_wave_1_price ? Number(form.single_wave_1_price) : null,
      single_wave_1_sold_out: form.single_wave_1_sold_out ?? false,
      single_wave_2_price: form.single_wave_2_price ? Number(form.single_wave_2_price) : null,
      single_wave_2_sold_out: form.single_wave_2_sold_out ?? false,
      single_wave_3_price: form.single_wave_3_price ? Number(form.single_wave_3_price) : null,
      single_wave_3_sold_out: form.single_wave_3_sold_out ?? false,
  ...
  const handleEdit = (event: any) => {
    ...
    setForm({
      ...
      single_wave_1_price: event.single_wave_1_price ?? '',
      single_wave_2_price: event.single_wave_2_price ?? '',
      single_wave_3_price: event.single_wave_3_price ?? '',
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error: resError } = await supabase.from('reservations').delete().eq('event_id', id)
  ...
  const makeWaveHandler = (type: 'single' | 'standing' | 'backstage' | 'vip') =>
    async (event: any) => {
      if (isViewer) return
  ...
  const handleSingleWaveAction = makeWaveHandler('single')
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#0a0f1e', padding: '0', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '96px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
          ...
          {isViewer && (
            <span style={{ background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.2)', color: '#94a3b8', fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', padding: '8px 16px', borderRadius: 8 }}>
              👁️ VIEW ONLY
            </span>
          )}
        </div>
        ...
        {msg && (
          <div style={{ background: 'rgba(39,174,96,0.1)', border: '1px solid rgba(39,174,96,0.25)', borderRadius: 12, padding: '12px 20px', color: '#27AE60', fontSize: 14, marginBottom: 24 }}>
            {msg}
          </div>
        )}
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: 20 }}>
                ...
                <div
                  ...
                  style={{ border: `2px dashed ${imagePreview ? 'rgba(46,117,182,0.5)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 14, overflow: 'hidden', cursor: 'pointer', backgroundColor: 'rgba(255,255,255,0.02)', minHeight: 140, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >
                  {imagePreview ? (
              ...
              {sectionLabel('🔵 SINGLE WAVES', '#60a5fa')}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 14 }}>
                <input style={{ ...inputStyle, borderColor: 'rgba(96,165,250,0.22)' }} type="number" placeholder="Single Wave 1 (optional)" value={form.single_wave_1_price} onChange={e => setForm({ ...form, single_wave_1_price: e.target.value })} />
                <input style={{ ...inputStyle, borderColor: 'rgba(96,165,250,0.22)' }} type="number" placeholder="Single Wave 2 (optional)" value={form.single_wave_2_price} onChange={e => setForm({ ...form, single_wave_2_price: e.target.value })} />
                <input style={{ ...inputStyle, borderColor: 'rgba(96,165,250,0.22)' }} type="number" placeholder="Single Wave 3 (optional)" value={form.single_wave_3_price} onChange={e => setForm({ ...form, single_wave_3_price: e.target.value })} />
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  border: 'none', padding: '14px 32px', borderRadius: 10, fontWeight: 700, fontSize: 14,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {events.map(event => (
            <div
              key={event.id}
              style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: 24, display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap', opacity: event.is_finished ? 0.6 : 1 }}
            >
              {event.image_url ? (
              ...
              <div style={{ flex: 1, minWidth: 220 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
                  <h3 style={{ color: event.is_finished ? '#555' : '#fff', fontSize: 16, fontWeight: 800, margin: 0, fontFamily: 'Poppins, sans-serif' }}>
                    {event.title}
                  </h3>
                  <span style={{ background: event.is_finished ? 'rgba(100,100,100,0.1)' : event.is_active ? 'rgba(39,174,96,0.1)' : 'rgba(255,255,255,0.04)', border: `1px solid ${event.is_finished ? '#333' : event.is_active ? 'rgba(39,174,96,0.3)' : 'rgba(255,255,255,0.08)'}`, color: event.is_finished ? '#555' : event.is_active ? '#27AE60' : '#555', padding: '2px 10px', borderRadius: 50, fontSize: 10, fontWeight: 700 }}>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 8 }}>
                  {event.single_wave_1_price != null && (
                    <div>
                      <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: 9, letterSpacing: '2px', fontWeight: 700, margin: '0 0 4px' }}>SINGLE</p>
                      {[1, 2, 3].map(w => event[`single_wave_${w}_price`] != null && (
                        <p key={w} style={{ color: event[`single_wave_${w}_sold_out`] ? '#555' : '#60a5fa', fontSize: 12, fontWeight: 600, margin: '1px 0', textDecoration: event[`single_wave_${w}_sold_out`] ? 'line-through' : 'none' }}>
                          W{w}: {event[`single_wave_${w}_price`]} EGP {event[`single_wave_${w}_sold_out`] ? '✗' : '●'}
                        </p>
                      ))}
                    </div>
                  )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, minWidth: 220 }}>
                ...
                {!isViewer && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 4 }}>
                    {[
                      { label: '🔵 SINGLE WAVE', fn: handleSingleWaveAction, clr: '#60a5fa', bg: 'rgba(96,165,250,0.08)' },
                      ...
                      <button key={label} onClick={() => fn(event)} style={{ background: bg, border: `1px solid ${clr}50`, color: clr, padding: '7px 11px', borderRadius: 8, fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif', letterSpacing: '1px' }}>
                        {label} →
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {!isViewer && (
                    <button onClick={() => handleFinish(event.id, event.is_finished)} style={{ background: event.is_finished ? 'rgba(255,255,255,0.03)' : 'rgba(39,174,96,0.08)', border: `1px solid ${event.is_finished ? 'rgba(255,255,255,0.06)' : 'rgba(39,174,96,0.25)'}`, color: event.is_finished ? '#555' : '#27AE60', padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                      {event.is_finished ? 'Unfinish' : '🏁 Finish'}
                    </button>
                  )}

                  {!isViewer && !event.is_finished && (
                    <button onClick={() => toggleActive(event.id, event.is_active)} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.3)', padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                      {event.is_active ? 'Hide' : 'Show'}
                    </button>
                  )}

                  <button onClick={() => router.push(`/dashboard/events/${event.id}/summary`)} style={{ background: 'rgba(240,165,0,0.08)', border: '1px solid rgba(240,165,0,0.25)', color: '#F0A500', padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                    Summary
                  </button>

                  <button onClick={() => router.push(`/dashboard/events/${event.id}/guest-list`)} style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.25)', color: '#60a5fa', padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                    Guest List
                  </button>

                  {!isViewer && (
                    <button onClick={() => handleEdit(event)} style={{ background: 'rgba(46,117,182,0.08)', border: '1px solid rgba(46,117,182,0.25)', color: '#2E75B6', padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                      Edit
                    </button>
                  )}

                  {!isViewer && (
                    <button onClick={() => handleDelete(event.id)} style={{ background: 'rgba(231,76,60,0.08)', border: '1px solid rgba(231,76,60,0.25)', color: '#E74C3C', padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                      Delete
                    </button>
                  )}
                </div>
