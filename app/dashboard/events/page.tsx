...
export default function DashboardEvents() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (ticketsError) {
      alert('Error deleting tickets: ' + ticketsError.message)
      setLoading(false)
      return
    }
    ...
    if (resError) {
      alert('Error deleting reservations: ' + resError.message)
      setLoading(false)
      return
    }
  ...
  const makeWaveHandler = (type: 'single' | 'standing' | 'backstage' | 'vip') =>
    async (event: any) => {
      ...
      if (w1p == null) {
        ...
        await supabase
          .from('events')
          .update({ [`${type}_wave_1_price`]: price, [`${type}_wave_1_sold_out`]: false })
          .eq('id', event.id)
        await load()
        return
      }
      ...
      if (!w1s) {
        ...
        await supabase
          .from('events')
          .update({ [`${type}_wave_1_sold_out`]: true, [`${type}_wave_2_price`]: price, [`${type}_wave_2_sold_out`]: false })
          .eq('id', event.id)
        await load()
        return
      }
      ...
      if (w1s && w2p != null && !w2s) {
        ...
        await supabase
          .from('events')
          .update({ [`${type}_wave_2_sold_out`]: true, [`${type}_wave_3_price`]: price, [`${type}_wave_3_sold_out`]: false })
          .eq('id', event.id)
        await load()
        return
      }
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '96px 24px 80px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 36, color: '#fff', margin: '8px 0 0', letterSpacing: '-1px' }}>
              Events
            </h1>
          </div>
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: 20 }}>
                <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 10, letterSpacing: '2px', fontWeight: 700, margin: '0 0 10px' }}>
                  EVENT IMAGE
                </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 14 }}>
                <input
                  style={{ ...inputStyle, borderColor: 'rgba(96,165,250,0.22)' }}
                  type="number"
                  placeholder="Single Wave 1 (optional)"
                  value={form.single_wave_1_price}
                  onChange={e => setForm({ ...form, single_wave_1_price: e.target.value })}
                />
                <input
                  style={{ ...inputStyle, borderColor: 'rgba(96,165,250,0.22)' }}
                  type="number"
                  placeholder="Single Wave 2 (optional)"
                  value={form.single_wave_2_price}
                  onChange={e => setForm({ ...form, single_wave_2_price: e.target.value })}
                />
                <input
                  style={{ ...inputStyle, borderColor: 'rgba(96,165,250,0.22)' }}
                  type="number"
                  placeholder="Single Wave 3 (optional)"
                  value={form.single_wave_3_price}
                  onChange={e => setForm({ ...form, single_wave_3_price: e.target.value })}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 14 }}>
                <input
                  ...
                  onChange={e => setForm({ ...form, standing_wave_1_price: e.target.value })}
                />
                <input
                  style={inputStyle}
                  type="number"
                  placeholder="Wave 2 (optional)"
                  value={form.standing_wave_2_price}
                  onChange={e => setForm({ ...form, standing_wave_2_price: e.target.value })}
                />
                <input
                  style={inputStyle}
                  type="number"
                  placeholder="Wave 3 (optional)"
                  value={form.standing_wave_3_price}
                  onChange={e => setForm({ ...form, standing_wave_3_price: e.target.value })}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 14 }}>
                <input
                  style={inputStyle}
                  type="number"
                  placeholder="Wave 1 (optional)"
                  value={form.backstage_wave_1_price}
                  onChange={e => setForm({ ...form, backstage_wave_1_price: e.target.value })}
                />
                <input
                  style={inputStyle}
                  type="number"
                  ...
                  onChange={e => setForm({ ...form, backstage_wave_2_price: e.target.value })}
                />
                <input
                  style={inputStyle}
                  type="number"
                  ...
                  onChange={e => setForm({ ...form, backstage_wave_3_price: e.target.value })}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
                <input
                  style={{ ...inputStyle, borderColor: 'rgba(240,165,0,0.2)' }}
                  type="number"
                  placeholder="VIP Wave 1 (optional)"
                  value={form.vip_wave_1_price}
                  onChange={e => setForm({ ...form, vip_wave_1_price: e.target.value })}
                />
                <input
                  style={{ ...inputStyle, borderColor: 'rgba(240,165,0,0.2)' }}
                  type="number"
                  placeholder="VIP Wave 2 (optional)"
                  value={form.vip_wave_2_price}
                  onChange={e => setForm({ ...form, vip_wave_2_price: e.target.value })}
                />
                <input
                  style={{ ...inputStyle, borderColor: 'rgba(240,165,0,0.2)' }}
                  type="number"
                  placeholder="VIP Wave 3 (optional)"
                  value={form.vip_wave_3_price}
                  onChange={e => setForm({ ...form, vip_wave_3_price: e.target.value })}
                />
              </div>

              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                <input type="checkbox" id="active" checked={form.is_active} onChange={e => setForm({ ...form, is_active: e.target.checked })} />
                <label htmlFor="active" style={{ color: 'rgba(255,255,255,0.3)', fontSize: 13, cursor: 'pointer' }}>
                  Visible to public
                </label>
              </div>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 220 }}>
                ...
                {event.transfer_number && (
                  <p style={{ color: '#F0A500', fontSize: 12, fontWeight: 600, margin: '2px 0 6px' }}>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 8 }}>
                  {event.single_wave_1_price != null && (
                    <div>
                      ...
                      {[1, 2, 3].map(
                        w =>
                          event[`single_wave_${w}_price`] != null && (
                            <p
                              key={w}
                              style={{
                                color: event[`single_wave_${w}_sold_out`] ? '#555' : '#60a5fa',
                                fontSize: 12,
                                fontWeight: 600,
                                margin: '1px 0',
                                textDecoration: event[`single_wave_${w}_sold_out`] ? 'line-through' : 'none',
                              }}
                            >
                  ...
                  <div>
                    ...
                    {[1, 2, 3].map(
                      w =>
                        event[`standing_wave_${w}_price`] != null && (
                          <p
                            key={w}
                            style={{
                              color: event[`standing_wave_${w}_sold_out`] ? '#555' : '#F0A500',
                              fontSize: 12,
                              fontWeight: 600,
                              margin: '1px 0',
                              textDecoration: event[`standing_wave_${w}_sold_out`] ? 'line-through' : 'none',
                            }}
                          >
                  ...
                  {event.backstage_wave_1_price != null && (
                    <div>
                      ...
                      {[1, 2, 3].map(
                        w =>
                          event[`backstage_wave_${w}_price`] != null && (
                            <p
                              key={w}
                              style={{
                                color: event[`backstage_wave_${w}_sold_out`] ? '#555' : '#8b5cf6',
                                fontSize: 12,
                                fontWeight: 600,
                                margin: '1px 0',
                                textDecoration: event[`backstage_wave_${w}_sold_out`] ? 'line-through' : 'none',
                              }}
                            >
                  ...
                  {event.vip_wave_1_price != null && (
                    <div>
                      ...
                      {[1, 2, 3].map(
                        w =>
                          event[`vip_wave_${w}_price`] != null && (
                            <p
                              key={w}
                              style={{
                                color: event[`vip_wave_${w}_sold_out`] ? '#555' : '#F0A500',
                                fontSize: 12,
                                fontWeight: 700,
                                margin: '1px 0',
                                textDecoration: event[`vip_wave_${w}_sold_out`] ? 'line-through' : 'none',
                              }}
                            >
