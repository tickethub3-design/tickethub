...
const url = await QRCode.toDataURL(ticketUrl, {
  ...
  if (loading) return (
    <main style={{ minHeight: '100vh', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
  ...
  if (!ticket) return (
    <main style={{ minHeight: '100vh', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <Link href="/profile" style={{ display: 'inline-block', marginTop: 20, color: '#64748b', fontSize: 13, textDecoration: 'none' }}>← Back to Profile</Link>
      </div>
    </main>
  )

  const tc          = typeConfig[ticket.ticket_type] || typeConfig.standing
  ...
  const eventDate   = ticket.events?.date
    ? new Date(ticket.events.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' })
    : 'TBA'
  ...
  return (
    <main style={{
      ...
      <Link href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 36 }}>
        <div style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg, #1A3C5E, #2E75B6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }}>🎟️</div>
      ...
      <div ref={ticketRef} style={{
        ...
        <div style={{ width: 1, borderLeft: '2px dashed #e2e8f0', position: 'relative', flexShrink: 0 }}>
          <div style={{ position: 'absolute', top: -13, left: -13, width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg, #0f172a, #1e1b4b)' }} />
          <div style={{ position: 'absolute', bottom: -13, left: -13, width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg, #0f172a, #1e1b4b)' }} />
        </div>
        ...
        <div style={{ flex: 1, padding: '24px 22px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            ...
            <div style={{
              ...
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: ticket.checked_in ? '#22c55e' : '#38bdf8' }} />
              <span style={{ color: ticket.checked_in ? '#15803d' : '#0369a1', fontSize: 9, fontWeight: 800, letterSpacing: '1.5px' }}>
        ...
        {/* ── RIGHT: QR ── */}
        <div style={{ width: 170, flexShrink: 0, background: '#f8fafc', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 14px', gap: 10 }}>
          <p style={{ color: '#94a3b8', fontSize: 8, letterSpacing: '2.5px', fontWeight: 700, margin: 0, textAlign: 'center' }}>SCAN AT ENTRANCE</p>
          ...
          <p style={{ color: '#cbd5e1', fontSize: 7, fontFamily: 'monospace', textAlign: 'center', margin: 0, wordBreak: 'break-all', lineHeight: 1.5, padding: '0 4px' }}>
