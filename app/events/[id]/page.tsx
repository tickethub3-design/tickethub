  ...
  return (
    <button
      ...
      style={{
        ...
        opacity: info.soldOut ? 0.75 : 1,
      }}
      onMouseEnter={e => {
        if (info.soldOut) return
        e.currentTarget.style.borderColor = 'rgba(46,117,182,0.35)'
        e.currentTarget.style.transform = 'translateY(-2px)'
        e.currentTarget.style.boxShadow = '0 10px 24px rgba(46,117,182,0.12)'
      }}
      onMouseLeave={e => {
        e.currentTarget.style.borderColor = 'rgba(46,117,182,0.12)'
    ...
    >
      ...
      {info.currentPrice != null && !info.soldOut ? (
        ...
        <p style={{ color: '#ef4444', fontSize: 15, fontWeight: 800, margin: 0, letterSpacing: '1px' }}>
          SOLD OUT
        </p>
      )}
    </button>
...
export default function EventPage() {
  ...
  if (loading) return (
    <main style={{ minHeight: '100vh', backgroundColor: '#0a0f1e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: 'rgba(255,255,255,0.2)', fontFamily: 'Inter, sans-serif', letterSpacing: '3px', fontSize: 12 }}>
        LOADING...
      </p>
    </main>
  )
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#0a0f1e', fontFamily: 'Inter, sans-serif' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(10,15,30,0.9)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(46,117,182,0.12)', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: 'linear-gradient(135deg, #1A3C5E, #2E75B6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }}>
            🎟️
          </div>
        ...
        <div className="nav-desktop" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <Link href="/events" style={{ color: 'rgba(255,255,255,0.55)', textDecoration: 'none', fontSize: 14, padding: '8px 14px', borderRadius: 8 }}>
            Events
          </Link>
          <Link href="/#about" style={{ color: 'rgba(255,255,255,0.55)', textDecoration: 'none', fontSize: 14, padding: '8px 14px' }}>
            About
          </Link>
          <Link href="/#contact" style={{ color: 'rgba(255,255,255,0.55)', textDecoration: 'none', fontSize: 14, padding: '8px 14px' }}>
            Contact
          </Link>
          <Link href="/auth/login" style={{ color: 'rgba(255,255,255,0.75)', textDecoration: 'none', fontSize: 14, fontWeight: 600, padding: '8px 18px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', marginLeft: 12 }}>
            Login
          </Link>
          <Link href="/auth/register" style={{ color: '#fff', textDecoration: 'none', fontSize: 14, fontWeight: 600, padding: '9px 20px', borderRadius: 8, background: 'linear-gradient(135deg, #1A3C5E, #2E75B6)', marginLeft: 4 }}>
            Sign Up
          </Link>
        </div>
      </nav>
      ...
      <div className="event-content">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
          {event.is_finished && (
            <span style={{ background: 'rgba(100,100,100,0.15)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.3)', padding: '5px 16px', borderRadius: 999, fontSize: 10, fontWeight: 700, letterSpacing: '2px' }}>
              🏁 EVENT ENDED
            </span>
          )}
          {allSoldOut && !event.is_finished && (
            <span style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.4)', color: '#ef4444', padding: '5px 16px', borderRadius: 999, fontSize: 10, fontWeight: 700, letterSpacing: '2px' }}>
              SOLD OUT
            </span>
          )}
        </div>
        ...
        <div className="ticket-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 32 }}>
          {!noStanding && <TicketCard label="STANDING" icon="🧍" info={standing} onClick={scrollToBooking} />}
          {!noVip && <TicketCard label="VIP" icon="⭐" info={vip} onClick={scrollToBooking} />}
          {!noBackstage && <TicketCard label="BACKSTAGE" icon="🎭" info={backstage} onClick={scrollToBooking} />}
        </div>
        ...
        {event.description && (
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(46,117,182,0.1)', borderRadius: 18, padding: 26, marginBottom: 32 }}>
            <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: 10, letterSpacing: '3px', fontWeight: 700, margin: '0 0 14px' }}>
              ABOUT THIS EVENT
            </p>
            <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 15, lineHeight: 1.9, margin: 0, whiteSpace: 'pre-wrap' }}>
              {event.description}
            </p>
          </div>
        )}
        ...
        <div id="book-now-section">
          {event.is_finished ? (
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, padding: 36, textAlign: 'center' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13, letterSpacing: '2px', fontWeight: 700, margin: 0 }}>
                THIS EVENT HAS ENDED
              </p>
            </div>
          ) : !event.is_active ? (
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, padding: 36, textAlign: 'center' }}>
              <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13, letterSpacing: '2px', margin: 0 }}>
            ...
            <div style={{ background: 'rgba(239,68,68,0.05)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 18, padding: 36, textAlign: 'center' }}>
              <p style={{ fontSize: 40, margin: '0 0 12px' }}>❌</p>
              <p style={{ color: '#ef4444', fontSize: 13, letterSpacing: '2px', fontWeight: 700, margin: 0 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(46,117,182,0.12)', borderRadius: 18, padding: 36, textAlign: 'center' }}>
              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 14, margin: '0 0 22px' }}>
                You need to be logged in to book a ticket.
              </p>
              <Link
                href={`/auth/login?redirect=/events/${event.slug}/reserve`}
